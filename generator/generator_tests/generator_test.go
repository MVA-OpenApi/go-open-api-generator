...
func TestGenerationOfProjectJson(t *testing.T) {
	config := generator.GeneratorConfig{OpenAPIPath: "examples/stores.json", OutputPath: "build", ModuleName: "build"}
	err := generator.GenerateServer(config)
	if err != nil {
		return
	}
...
func TestGenerationOfProjectYaml(t *testing.T) {
	config := generator.GeneratorConfig{OpenAPIPath: "examples/stores.yaml", OutputPath: "build", ModuleName: "build"}
	err := generator.GenerateServer(config)
	if err != nil {
		return
	}
...
func TestDeletionOfProjects(t *testing.T) {
	...
	fs.DeleteFolderRecursively("build")
	config = generator.GeneratorConfig{OpenAPIPath: "stores.yaml", OutputPath: "build", ModuleName: "mock"}
	err1 := generator.GenerateServer(config)
	if err1 != nil {
		return
	}
